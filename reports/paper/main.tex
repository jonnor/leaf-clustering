...
%% Appendix A
\newpage
\section{Results for all datasets and methods}
\label{app:proofs}
%\clearpage
\begin{figure}[H]
\centering
\vspace{-1em}  % reduce space above
\includegraphics[height=0.95\textheight,keepaspectratio]{reports/figures/perf-pareto-datasets.png}
%\vspace{-0.5em}  % reduce space below
\caption{Complete set of results across all datasets. Pareto frontier for each compression strategy is shown as a line.}
\vspace{-1em}  % reduce space below caption
\label{fig:performance_pareto_datasets}
\end{figure}

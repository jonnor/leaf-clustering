...
def plot_pareto_front(results,
    ...
    pareto_params = dict(
        ...
        min_performance=pareto_cut,
    )

    global_pf = find_pareto_front(results, **pareto_params)
    ...
    def _plot_front(color, label):
        if pareto_global:
            ...
            sub = find_pareto_front(results[results[hue] == label], **pareto_params)

        sub = sub.sort_values(cost_metric)
